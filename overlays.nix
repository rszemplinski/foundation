[
  ...
  (self: super:
    with super;
    with mylib; {
      nix-wrapped = if isNixOS then
        nix
      else
        wrapBins nix ''
          mkdir -p ~/.local/share/nix
          export NIX_CONFIG=$(< ${
            writeText "nix.conf" cfg.nixConfBase
          })$'\n'$NIX_CONFIG
          exec "$exePath" "$@"
        '';
  ...
  (self: super:
    ...
    with mylib; {
      ...
      inherit (self.nle-cfg.pkgs.poetry-env.python.pkgs)
        pur emborg git-remote-codecommit;
      ...
      contentAddressed = mapAttrs (_: pkg:
        if pkg ? overrideAttrs then
          pkg.overrideAttrs (_: { __contentAddressed = true; })
        else
          pkg) pkgs;
      switch = stdenv.mkDerivation {
        ...
        installPhase = ''
          mkdir -p $out/bin
          ln -s ${
            self.switch-to-configuration.scripts.${builtAsHost}.noa
          }/bin/switch $out/bin
          ${optionalString isNixOS "ln -s ${
            cfg.nixosConfigurations.${builtAsHost}
          } $out/nixos-configuration"}
      ...
      bin-aliases = alias {
        ...
        hmg =
          "cd ~/cfg && git dfo && git rebase --autostash origin/$(git branch-name)";
        hmp = "git -C ~/cfg cap";
        nou = "hmg && noa";
        ...
        nb = ''
          pkg=$1 && shift; nix build $(echo "$pkg" | sed -E "s@^|,@ $HOME/cfg#@g")'';
        ns = ''
          pkg=$1 && shift; nix shell $(echo "$pkg" | sed -E "s@^|,@ $HOME/cfg#@g")'';
  ...
  (self: super:
    ...
    with mylib;
    let
      extra-packages = mapAttrs (n: f:
        callPackage f (mylib // pkgs // rec {
          name = "${pname}-${version}";
          pname = n;
