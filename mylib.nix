...
cli // generators // lib // builtins // rec {
  ...
  exe = pkg:
    let
      b = (pkg.meta or { }).mainProgram or (removePrefix "node_"
  ...
  mapLines = f: s:
    ...
    (splitString "\n" s);
  words = string:
    filter (x: isString x && x != "") (split "[[:space:]]+" string);
  ...
  nixpkgs-branch =
    ...
    in if length urlParts == 3 then elemAt urlParts 2 else "master";
  excludeLines = f: text:
    concatStringsSep "\n" (filter (x: !f x) (splitString "\n" text));
  unpack = src:
    stdenv.mkDerivation {
      src = if src ? url && src ? sha256 then
    ...
    if isString name then
      ...
      let
        ...
        pre = if any (s: hasInfix s arg) [ "&&" "||" ";" "|" "\n" ] then
          ...
          "exec";
        post =
          if any (s: hasInfix s arg) [ ''"$@"'' "\n" ] then "" else ''"$@"'';
      in writeBashBin name "${pre} ${cmd} ${post}"
    else
      mapAttrs alias name;
  mkDmgPackage = pname: src:
    stdenv.mkDerivation {
  ...
  dmgOverride = name: pkg:
    ...
    if isDarwin then
      assert checkVersion; (mkDmgPackage name src) // { originalPackage = pkg; }
    else
      pkg;
  importNixpkgs = src:
    let
      realSource = if isDerivation src then
        src
      else
        fetchFromGitHub {
          owner = "NixOS";
  ...
  buildDir = paths:
    ...
    in runCommand "source" { } ''
      mkdir $out
      ${cmds}'';
  buildDirExcept = path: except:
    buildDir (map (x: path + ("/" + x))
      (filter (x: !any (y: y == x) (toList except))
        (attrNames (readDir path))));
  pathAdd = pkgs: "export PATH=${makeBinPath (toList pkgs)}:$PATH";
  makeScript = name: script:
    writeBashBin name (if isDerivation script then
      ''exec ${script} "$@"''
    else
      ''
        set -e
  ...
  joinStrings = sep: f: g:
    concatMapStringsSep sep
    (s: if isString s then f s else g (head s) (lib.last s));
  joinLines = joinStrings "\n";
  override = x: y:
    if y ? _replace then
      y._replace
    else if y ? _append then
      x + y._append
    else if isList x && isList y then
      x ++ y
    else if isDerivation x && isPath y && pathExists y then
      y
    else if isDerivation x && isPath y && !pathExists y then
      x
      ...
      x (attrs:
        mapAttrs (n: v: if hasAttr n attrs then override attrs.${n} v else v) y)
  ...
  mapDirEntries = f: dir:
    listToAttrs
    (filter (x: x != null && x != { }) (mapAttrsToList f (readDir dir)));
  import' = path:
    let
      importEntry = entry: type:
        if type == "directory" then
          ...
          in if isAttrs default then
            default // entries
          else if isFunction default then
            entries // {
              __functor = _: default;
              __functionArgs = functionArgs default;
            }
          else
            default
        else if hasSuffix ".nix" entry then
          import entry
        else
          entry;
    in importEntry path "directory";
  fixSelfWith = f: x: let self = f (x // { inherit self; }); in self;
